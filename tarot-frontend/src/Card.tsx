...
export function CardBack() {
  ...
  const findPoint =
    (cx: number, cy: number, rad: number, cornerGrad: number): [number, number] => {
  ...
  let circlePoints = Array(6).fill(null).map((_, i) => findPoint(centerX, centerY, radius, 60 * i))

  let circles = circlePoints.map(([x, y], i) => <circle cx={x} cy={y} r={radius} fill="transparent" key={i} />);
