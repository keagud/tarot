...
function Card({ title, image }: CardProps) {
  ...
  const commonCardStyles =
    'w-full border-tarot-slate rounded-xl border-8 shadow-lg shadow-tarot-black';
  ...
  return (
    <div className="w-full m-20 relative h-3/5 max-w-[700px]">
      <div
        className={` w-full absolute border border-tarot-green  card ${isFlipped ? 'flipped' : ''
          }  `}
        onClick={toggleFlipped}
      >
        <div className="card-face front ">
          {' '}
          <img src={cardFrontImage} className={commonCardStyles} />
        </div>
        <div className="card-face back">
          <img src={cardBackImage} className={commonCardStyles} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      <div className="font-sans m-10 leading-loose  text-tarot-jet">
        <div>
          <h1 className="underline text-center outline-tarot-yellow"
            style={{ fontFamily: "Vulnus", fontSize: "3.5em" }}>{title.toUpperCase()}</h1>
        </div>

        <FadeInText>
          <div>
            <br />
            <div className='text-center m-10' style={{ fontFamily: "wollstonecraft", fontSize: '1.7em' }}>
              <p> {meaning}</p>
            </div>
            <br />
            <div className="m-10 overflow-scroll" style={{ fontFamily: "wollstonecraft", fontSize: "1.7em", fontStyle: 'italic' }}>
              <FadeInText> <><p> {description} </p> </></FadeInText>
            </div>
          </div>
        </FadeInText>
...
const getMask = ([start, end]: [number, number], step: number = 1, gap: number = 50): [number, number] => {
...
function FadeInText({ children }: { children: ReactNode }) {
  ...
  return (<><div style={{ maskImage: `linear-gradient(0deg, transparent ${gradientStart}%, black ${gradientEnd}%` }} >
    {children}
  </div>
